...
async def run_cli_game(
    ...
    if models:
        ...
        config = GameConfig(
            ...
            num_voting_rounds=num_voting_rounds,
...
def main():
    ...
    parser.add_argument(
        "--voting-rounds", type=int, default=None,
        help="Number of voting rounds (default: min(2, imposters), max: imposters)"
    )
    parser.add_argument(
        "--models",
    ...
    asyncio.run(run_cli_game(
        ...
        num_voting_rounds=args.voting_rounds,
        models=args.models
    ))
