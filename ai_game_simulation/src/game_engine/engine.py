...
class GameConfig:
    ...
    # num_voting_rounds: Fixed number of voting rounds (eliminations).
    # - Max allowed: num_imposters (can't need more rounds than imposters to catch)
    # - Default: min(2, num_imposters) - 2 rounds unless only 1 imposter
    # - Set to None to use default logic
    num_voting_rounds: Optional[int] = None
    ...
    def __post_init__(self):
        """Apply default logic for computed fields"""
        # Default voting rounds: min(2, num_imposters)
        if self.num_voting_rounds is None:
            self.num_voting_rounds = min(2, self.num_imposters)

        # Validate: voting rounds cannot exceed num_imposters
        if self.num_voting_rounds > self.num_imposters:
            self.num_voting_rounds = self.num_imposters
...
class GameEngine:
    ...
    def __init__(self, config: GameConfig, openrouter_client: OpenRouterClient, visual_mode: bool = False):
        ...
        # Voting system (see _calculate_results() docstring for full architecture notes)
        # LEGACY: self.all_votes - batch voting where all players vote at once (DEPRECATED)
        # CURRENT: self.sequential_votes - players vote one at a time with visibility
        self.all_votes: List[VoteRecord] = []  # LEGACY - no longer populated
        self.sequential_votes: List[SingleVoteRecord] = []  # CURRENT - active voting system
        self.eliminated_players: List[str] = []  # Updated in real-time during sequential voting
    ...
    async def _execute_voting(self):
        ...
        Uses FIXED number of voting rounds set in config.num_voting_rounds.
        - Default: min(2, num_imposters) - gives 2 chances unless only 1 imposter
        - Max: num_imposters - can't need more rounds than imposters to catch
        - User-configurable via game setup UI

        The game CAN end early if:
        - All imposters are caught before all rounds complete
        - Imposters outnumber civilians (imposter victory)

        But it will NOT extend beyond the configured rounds even if civilians
        are eliminated by mistake. This is intentional game design.
        """

        if self.visual_mode and CLI_DISPLAY_AVAILABLE:
            print_voting_header()

        # Use configured voting rounds (validated in GameConfig.__post_init__)
        total_voting_rounds = self.config.num_voting_rounds
        ...
        for voting_round in range(1, total_voting_rounds + 1):
            ...
            for idx, player in enumerate(active_players):
                ...
                # DESIGN NOTE: Self-Voting Prevention
                # ===================================
                # Players cannot vote for themselves (excluded from valid_targets).
                # This is a deliberate game rule choice.
                #
                # Historical context: Earlier analysis documents (FINAL_ANALYSIS_2025-12-28.md)
                # reference self-voting behavior, but that was from a LEGACY batch voting system
                # (VoteResponse + self.all_votes). The current sequential voting system
                # (SingleVoteResponse + self.sequential_votes) explicitly prevents self-voting.
                #
                # If self-voting should ever be allowed as a strategic choice, remove the
                # `p.player_id != player.player_id` filter below.
    ...
    def _calculate_results(self) -> GameResult:
        """
        Determine game outcome based on eliminations during voting phase.

        ARCHITECTURE NOTE: Voting System Evolution
        ==========================================
        This codebase has TWO voting systems:

        1. LEGACY Batch Voting (VoteResponse + self.all_votes)
           - All players vote simultaneously for multiple suspects
           - Votes tallied at end to determine eliminations
           - Schema: VoteResponse with votes: List[str]
           - Status: DEPRECATED - self.all_votes is no longer populated

        2. CURRENT Sequential Voting (SingleVoteResponse + self.sequential_votes)
           - Players vote one at a time, seeing previous votes
           - One elimination per voting round, immediate
           - Schema: SingleVoteResponse with vote: str (singular)
           - Eliminations stored directly in self.eliminated_players

        The legacy vote tallying code below is preserved for backwards compatibility
        but will always find self.all_votes empty in the current flow. Eliminations
        are now tracked in real-time via self.eliminated_players during _execute_voting().
        ...
        # LEGACY: Tally batch votes (self.all_votes)
        # This code path is no longer active but preserved for potential future use.
        # In the current sequential voting flow, self.all_votes is always empty.
        vote_counts = defaultdict(int)
        for vote_record in self.all_votes:
            for suspect in vote_record.voted_for:
                vote_counts[suspect] += 1

        # LEGACY: Top voted players with tie handling
        if vote_counts:
            max_votes = max(vote_counts.values())
            tied_players = [p for p, v in vote_counts.items() if v == max_votes]
        ...
        # Combine legacy batch eliminations with sequential eliminations
        # In current flow, eliminated_by_vote is always empty, so this equals self.eliminated_players
